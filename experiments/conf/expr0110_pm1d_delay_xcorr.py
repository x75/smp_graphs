...
run for {0} time steps. A cross-correlation scan is then performed on
the motor variables $m$ versus the sensor variables $s_0$. The scan
consists of computing the Pearson correlation coefficient of $m_i$ and
$s_i$ for a set of preset temporal offsets.  The output is generated
by incrementally shifting the destination variable in the pair by one
time step into the past and then computing the correlation
coefficient. The scanning range of temporal offsets in this example is
going from {1} to {2}. The output of the scan correctly determines a
delay of 2 time steps, which is the maximum correlation over all
possible shifts. A shift of zero is not included as a minimum
motor-sensor delay of one time step is assumed by the underlying
representation of time and causality.""".format(

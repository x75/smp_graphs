...
from smp_graphs.utils_conf import get_systemblock, get_subplots_expr0120
...
scan_plot_inputs['d2'] = {'bus': 'mi_ll1_ll0/mi', 'shape': (dim_s1, dim_m0, scanlen)}
scan_plot_inputs['d3'] = {'bus': 'mi_ll2_ll0/mi', 'shape': (dim_s1, dim_s1, scanlen)}
scan_plot_inputs['m2m'] = make_input_matrix_ndim(id = 'xcorr_m2m', xdim = dim_m0, ydim = dim_s0, with_t = True, scan = (scanstart, scanstop))['d3']
...
graph = OrderedDict([
    ...
    ('plot0_meas_l0', {
        ...
        'params': {
            ...
            panel contains the graph of the first order state
            variable, the velocity. The dissipative term of the
            velocity (e.g. friction) keeps the velocity within bounds
            while it is still dominated by the remaining inertia. The
            dissipation parameter is set to a value of
            {0}.""".format(robot1['params']['dims']['s1']['dissipation']),
            'inputs': {
                's0': {'bus': 'robot1/s0', 'shape': (dim_s0, numsteps)},
                's0p': {'bus': 'pre_l0/pre', 'shape': (dim_m0, numsteps)},
                's1': {'bus': 'robot1/s1', 'shape': (dim_s1, numsteps), 'xtwin': True},
            },
            'subplots': [
                [
                    {
    ...
    ('xcorr_m2s', {
        ...
        'params': {
            # 'debug': True,
            'blocksize': numsteps,
            'inputs': {'y': {'bus': 'pre_l0/pre', 'shape': (dim_s0, numsteps)}, 'x': {'bus': 'robot1/s1', 'shape': (dim_s1, numsteps)}},
            'shift': (scanstart, scanstop),
            'outputs': {'xcorr': {'shape': (dim_m0, dim_s1, scanlen)}},
        }
    }),
    ...
    ('plot_xcorr', {
        ...
        'params': {
            ...
            'wspace': 0.5,
            'hspace': 0.5,
            'subplots': get_subplots_expr0120(dim_s0, dim_m0, dim_s1, scanlen, scanstart, scanstop),
        },
    }),
